...
The poem editor, practice worksheet, and scale studio share named wording controls. Save requires a new unique name, and the two starting originals are protected from deletion. Import a photographed handwritten poem with its exact transcription and an editable calligrapher credit; reviewed ink becomes a poem-specific face in the scale and continuous foil viewers. Matching original handwriting is preferred automatically, repeated words retain their own strokes, and scale-study exports can carry the original scan and mapping. See [named poems and scanned calligraphy](docs/scanned-calligraphy.md) for the workflow, storage, and segmentation limits.
...
- **Sculpture & scales** (`/`, with detailed controls at `/scales`): hop between nine starting shapes and recent working versions, resize the archival construction or actual printable maquette, and reflow lettering with measured fonts. Tune local body width and depth from 0.5–2 times the source cross-section, linked by default or independently, while keeping the centerline and base fixed. Tune the plate silhouette, physical width-to-height ratio, clipped corners, taper, raised depth, and seeded variation. Cover mode keeps full cells and treats aspect as an approximate course-layout target; inset mode fits an exact aspect by shrinking faces. A matte clipped-wood version follows local visual proportions in the reference photographs, with a solid support body visible beneath the plates. Saved older studies retain their original shapes and inset behavior; the original eight presets retain their previous construction settings. Optional adaptive density changes plate counts with size and estimates row changes from local body girth. Body edits retain physical stock and lettering size; measured bounds and deterministic non-AI text mapping update with the surface. A page-local cache automatically reuses up to four recent geometries within a 64 MiB estimated budget; settings persist across reload while meshes rebuild. Eased camera motion and surface crossfades retain the working preview during remapping. Compare intermediate wood/printed support, backing, adhesive, metal, finish, overlap, and raised-face dimensions. Save a portable design with custom font bytes and construction notes. See the [scale studio guide](docs/scale-studio.md) for model distinctions and physical-fit limits.
- **Reference gallery** (`/references`): view or download all three explicitly approved complete original images, with byte counts and SHA-256 fingerprints.
- **Project instructions** (`/instructions`): compare the source sculpture, 180 mm maquette, and Letter-landscape paper masters with millimetre/inch display controls. Follow the making steps, fit aluminum to the large wooden sculpture, protect each material, then arrange the CoLab iani handoff. Section links (`#dimensions`, `#make`, `#large-sculpture`, `#pack`, `#send`) open the relevant step directly. A printable packing slip has blank recipient and shipment fields; confirm the address and receiving arrangements directly before mailing.
- **Endless inscription** (`/foil`): a silver skin follows the original body and head. Four rows repeat the complete poem around closed reading paths on the large body. Taller lettering in its UV master compensates for the long surface path so the script reads clearly on the foil. The shorter lower jaw keeps a separate eighteen-row inscription master. The page also shows the 180 mm printed form covered in marked foil. A scale selector switches the flat artwork between the large UV master and the small dimensioned marking sheet. Workshop downloads are grouped by scale, with separate foil kits and a smooth STL substrate for printing the small model.
...
The homepage combines scales, resizing, working versions, and a zoomable lettering proof. It links to making, lettering, reference originals, and the continuous foil edition. The foil page retains its dimensions, artwork, downloads, and script or plain-type reading view. Navigation and model controls retain readable labels on small screens; route changes update the page title and browser back navigation restores the scroll position.
...
[iani’s guide to aluminum on wood](https://muchadoaboutoneside.com/instructions#large-sculpture) plans varied, individually fitted aluminum scale plates for the actual sculpture. It covers supported body pieces, smaller pieces at tight turns, custom edge and jaw shapes, a neighboring paper-template group, plate IDs, and lettering kept clear of cuts and hidden overlap margins. Its downloadable shop notes use the same content as the page and record the measurements needed before metal production. The actual stock, wood finish, support, adhesive system, and indoor/outdoor conditions must be confirmed with a complete marked and fitted sample. Manufacturer guidance is linked in the guide. Fable’s copperplate brief and physical print templates are preserved.

The dimension cards retain the original construction-model bounds. The scale studio also compares actual bare body-and-jaw bounds with generated plate faces and sidewalls, excluding the archival square base from that body comparison. The print study includes its integrated plinth. Reference-photo crops and generated face ratios help compare the local wood-plate appearance; the 1.25 starting aspect is a visual assumption rather than dimensional calibration. Record the actual prepared support surface, local backing or leveling, adhesive, aluminum, finish, and any overlapping plate thickness before approving new scale patterns; then measure the finished clad envelope and minimum jaw/crossing clearances. The large preview and existing triangular kit include a provisional geometric cross-section offset from `shared/fabrication.ts`. That offset has no measured construction-layer specification. The old kit remains a conceptual reference while the new scale shapes and dimensions are established on the actual wood. The separate 180 mm print and foil system keeps its own dimensions and patterns.

## Six-foot indoor display

[The six-foot construction study](https://muchadoaboutoneside.com/six-foot) compares six ways to make a reusable indoor display for ordinary touch, with no climbing. Supported aluminum over a wood structure and faceted aluminum-composite panels are the first two systems to sample; thin flashing, steel framing, all-aluminum construction, and hard-coated foam remain conditional alternatives. The study includes dated raw-stock price examples, a downloadable plan, edge and panel sample checks, lettering reproofing, transport, and base-design decisions. The lowest finished cost depends on local material, tools, labor, and the cutting layout.

The finished target is **72 inches overall**, including the new skin and redesigned base. Source-derived proportions give a body about **40.4 inches wide × 12.0 inches deep** before new cladding. The original base scales to about 59.7 inches square as a reference; it is not the proposed indoor footprint. Conceptual body-and-jaw skin area is about **24.5–27.6 square feet**. A preliminary 35–40 square-foot stock allowance excludes the base and is not a nesting result. These values come from `shared/six-foot-study.ts` and a source-hash-checked geometry reference; tests recompute the source bounds and mesh areas. The existing large fabrication files are not a six-foot cutting kit.
...
Jill's supplied photograph is published byte for byte in the [reference gallery](https://muchadoaboutoneside.com/references), following Andrew's explicit approval on 2026-09-18. Its song text is not the inscription. The public prototype uses locally bundled **Great Vibes** under the [SIL Open Font License](public/fonts/OFL.txt), chosen for its slanted script and flourished capitals. It is an editable substitute for Jill's eventual hand-lettered master, not a claim that the font is her handwriting. Only the approved gallery copy is included in the public repository, website, and deployment package; the legacy `source/reference/` archive remains excluded. Publication does not change rights in third-party text or images.
...
GitHub's `.github/workflows/site.yml` runs a frozen install, checks, build, HTTP smoke, and headless browser tests on public pushes and pull requests. The private canonical archive also runs its Gitea workflow.
...
The public GitHub repository starts with a clean snapshot rather than the private archive's history. From a clean, reviewed commit in the canonical archive, run `bun run export:public --output /tmp/muchado-public-snapshot`. The exporter selects public source files, excludes private archives and host configuration, rejects symlinks and known restricted image bytes outside their explicitly approved gallery path and hash, and records the source commit and file hashes in `PUBLIC_SNAPSHOT.json`. It does not publish anything or copy Git history. Verify a frozen install, `bun run check`, `bun run build`, and `bun run smoke` in the exported snapshot before updating GitHub. Keep the GitHub checkout separate from the private archive; do not push the private repository's refs to GitHub.
